...
skills/                  # Source-of-truth skill definitions (6 first-party skills)
  ...
  <skill>/assets/        # Files for people, shipped with the skill but never loaded into agent context (best-minds-drill)
...
Each first-party skill has a dedicated README in `readme/` (`README-best-minds-optimizer.md`, `README-4d-mind-analyst.md`, `README-feature-list-mind.md`, `README-best-minds-drill.md`) plus Chinese translations (`*-CN.md`). The main `README.md` is the project overview and the only README at the repo root; its Chinese translation is `readme/README-CN.md`. `best-minds-setup` has no separate README — it is covered in `README.md` and in its own `SKILL.md`.
